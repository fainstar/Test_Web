        ...
        try:
            ...
            for question_data in questions:
                ...
                if normalized_data:
                    ...
                    if is_new:
                        success_count += 1
                        print(f"✅ 已新增題目 ID:{question_id}, 類型:{normalized_data['question_type']}")
                    else:
                        skip_count += 1
                else:
                    skip_count += 1
...
def normalize_question_format(question_data):
    ...
    try:
        ...
        if not options:
            print(f"⚠️  題目缺少選項: {question_text[:50]}...")
            return None
        ...
        if 'correct_answer' in question_data:
            ...
            if isinstance(ca, str):
                ...
                try:
                    ...
                    print(f"⚠️  找不到答案 '{ca}' 在選項 {options} 中")
                    return None
                ...
                for ans in ca_list:
                    if isinstance(ans, str):
                        try:
                            ...
                            print(f"⚠️  找不到答案 '{ans}' 在選項 {options} 中")
                            return None
                    elif isinstance(ans, int):
                        correct_answers.append(ans)
        ...
        print(f"📝 處理題目: {question_type}, 正確答案: {correct_answer}")
        return result
